...
例えば，Reynolds の実験<sup>[2]</sup>に代表されるような円管内流れは線形安定性解析で $\rm{Re}_\rm{L} \to \infty$ となりますが，これは我々の直観に反した結果です．
実際には $\rm{Re} \approx 2000$（大抵はこれより大きい Reynolds 数）で乱流に遷移します．
...
壁面剪断流の多くは亜臨界遷移に属し，乱流維持下限の大域的安定臨界 Reynolds 数 $\rm{Re}_\rm{G}$ 近傍では層流と乱流が時空間的に共存し，局在化した乱流が乱流パフ<sup>[2]</sup>や乱流縞<sup>[3,4]</sup>と呼ばれる特徴的なパターンを形成します．
